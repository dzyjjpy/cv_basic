...
//  Function:  NV21_2_RGB
//  Description: convert YUV(NV21) pointer image to RGB format(by jiapy)
//  @para data: YUV pointer
//  @para rgb: RGB pointer (RGB order)
//  @para width: image's width
//  @para height: image's height
void NV21_2_RGB(unsigned char* data, unsigned char* rgb, int width, int height)
{
    if(NULL==data || NULL==rgb)
    {
        //std::cout << "data or rgb pointer is empty" << std::endl;
        //exit(1);
    }
    int index = 0;
    ...
    for(int y=0; y< height; y++)
    {
        ...
        {
            // YYYYYYYYYVUVU
            unsigned int Y = ybase[x + y*width];
            unsigned int V = ubase[x/2 + (y/2)*width];
            unsigned int U = ubase[x/2 + (y/2)*width + 1];
